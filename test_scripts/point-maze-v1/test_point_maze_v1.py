import os
import numpy as np
from rlnav.point_maze import PointMazeV1
...
# Create environment
env = PointMazeV1(goal_conditioned=True)
...
# Run random policy
print("Generating random policy video...")
done = False
obs, _ = env.reset()
frames = [env.render()]
for _ in range(100):
...
# Run goal-conditioned random policy
print("Generating goal-conditioned random policy video...")
env = PointMazeV1(goal_conditioned=True)
env.reset()
done = False
obs, _ = env.reset()
