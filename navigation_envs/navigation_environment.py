...
class NavigationEnvironment:
    ...
    def __init__(self,
                 map_name: str = MapsIndex.EMPTY.value,
                 goal_conditioned: bool = True,
                 reset_anywhere: bool = True,
        ...
        self._maze_map = np.array(importlib.import_module("navigation_envs.maps." + self.map_name).maze_array)

        self._height, self._width = self._maze_map.shape
        self._coordinates_space = Box(low=np.array([.5 - self._width / 2, - (self._height / 2 - .5)]),
                                      high=np.array([self._width / 2 - .5, - (.5 - self._height / 2)]),)
        self.reset()
    ...
    def get_observation(self, x, y):
        ...
        If x and y are given, return the observation that we can expect at these coordinates.
        """

    def get_tile_type(self, x, y):
        return TileType(self._maze_map[y][x].item())
